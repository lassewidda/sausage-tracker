...
export async function analyzeImage(imageUrl: string): Promise<AnalysisResult> {
  ...
  if (parsed.exercise_type) {
    ...
    const exerciseType = validTypes.includes(parsed.exercise_type)
...
- Do NOT use markdown formatting, just plain text`;
  ...
  if (!block || block.type !== "text")
    return `💪 Welcome to PowerUp, ${data.playerName}! Your first workout is logged. Keep going!`;
  return block.text.trim();
}
...
- Do NOT use markdown formatting, just plain text`;
  ...
  const block = message.content.find((b) => b.type === "text");
  if (!block || block.type !== "text")
    return `💪 ${data.playerName.toUpperCase()}, ${data.context}`;
  return block.text.trim();
}
...
export async function generateChannelSummary(data: {
  ...
  const highlights =
    data.playerHighlights.length > 0
  ...
  const incompleteTeamLines =
    data.challengeInfo?.isGroupMode &&
    data.challengeInfo.incompleteTeams &&
    data.challengeInfo.incompleteTeams.length > 0
      ? data.challengeInfo.incompleteTeams
          .slice()
          .sort(
            (a, b) => b.bingoDone - a.bingoDone || (b.totalWorkouts ?? 0) - (a.totalWorkouts ?? 0),
          )
          .slice(0, 6)
          .map((t) => {
            const memberStatus =
              t.membersExerciseStatus && t.membersExerciseStatus.length > 0
                ? t.membersExerciseStatus
                    .map((m) => `${m.name} ${m.workouts}${m.exerciseMet ? "✓" : "✗"}`)
                    .join(" / ")
                : t.members.join(" & ");
            const totalWk =
              t.totalWorkouts !== undefined ? `, ${t.totalWorkouts} team workouts` : "";
...
- Challenge page: https://powerup.eliteprospects.com/challenge${
        data.challengeInfo.isGroupMode &&
        data.challengeInfo.completedTeams &&
        data.challengeInfo.completedTeams.length > 0
        ...
        !data.challengeInfo.isGroupMode &&
        data.challengeInfo.completedPlayers &&
        data.challengeInfo.completedPlayers.length > 0
          ? `\n- CHALLENGE COMPLETERS: ${data.challengeInfo.completedPlayers.join(", ")}`
          : ""
      }${
        !data.challengeInfo.isGroupMode &&
        data.challengeInfo.incompletePlayers &&
        data.challengeInfo.incompletePlayers.length > 0
          ? `\n- STILL IN PROGRESS: ${data.challengeInfo.incompletePlayers.join(", ")}`
          : ""
      }`
    : "";

  const isMonday = data.dayLabel === "Monday";

  const statsBlock = isMonday
    ? `It is Monday morning — day 1 of 7. There are no meaningful weekly stats yet; the week has barely begun. The personal goals on the books this week:
- ${data.totalPlayers} players have weekly goals set — they have until Sunday night to hit them${data.topStreak ? `\n- Current longest streak (carrying in from prior weeks): ${data.topStreak.player} with ${data.topStreak.weeks} consecutive goal weeks` : ""}`
    : `Stats this week:
...
${isMonday ? "- CRITICAL FOR MONDAY: It is the start of the week. Do NOT cite this-week activity as a scoreboard — there is no scoreboard yet on day 1. Do NOT count or comment on who has or hasn't logged so far. Do NOT note that nobody has hit their goal yet — of course they haven't, the week just started. Frame the week as the start line: goals are set, Sunday is the finish, the road is wide open." : "- Gently encourage those who haven't logged yet (without shaming)"}
...
${
  data.challengeInfo && data.dayLabel === "Friday"
    ? data.challengeInfo.isGroupMode
...
- Keep it casual and fun, plain text only (no markdown)`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens:
      data.challengeInfo &&
      (data.dayLabel === "Monday" ||
        data.dayLabel === "Friday" ||
        (data.dayLabel === "Wednesday" && data.challengeInfo.isGroupMode))
        ? 500
        : 300,
    messages: [{ role: "user", content: prompt }],
  });
  ...
  const block = message.content.find((b) => b.type === "text");
  if (!block || block.type !== "text")
    return `💪 PowerUp ${data.dayLabel} update: ${data.totalActivities} workouts logged by ${data.activePlayers} players this week!`;
  // Fix any mangled powerup URLs (Haiku sometimes garbles long URLs)
  const correctDomain = "https://powerup.eliteprospects.com";
  let text = block.text.trim().replace(/https?:\/\/powerup\.[a-z.]+\.com/g, correctDomain);
  ...
  for (const path of urlPaths) {
    const pattern = new RegExp(
      `(${correctDomain.replace(/\//g, "\\/")}\\/${path})([^\\s.,;!?)}\\]"'\\n/])`,
      "g",
    );
  ...
  text = text.replace(
    new RegExp(`(${correctDomain.replace(/\//g, "\\/")})([^\\s.,;!?)}\\]"'\\n/])`, "g"),
    "$1 $2",
  );
  return text;
}
