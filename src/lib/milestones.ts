...
export async function checkMilestones(ctx: MilestoneContext): Promise<void> {
  ...
  try {
    ...
    try {
      // Gather all data in parallel. Photo-bingo uploads share the meals
      // table but are not workouts — exclude them from all milestone counts.
      const [totalResult, weekResult, historyResult, leaderboardResult, gapResult, descResult] =
        await Promise.all([
          sql`SELECT COUNT(*)::int AS count FROM meals WHERE player_name = ${name} AND (exercise_type IS NULL OR exercise_type <> 'photo')`,
          sql`SELECT COUNT(*)::int AS count FROM meals WHERE player_name = ${name} AND week_key = ${weekKey} AND (exercise_type IS NULL OR exercise_type <> 'photo')`,
          sql`SELECT week_key, COUNT(*)::int AS total FROM meals WHERE player_name = ${name} AND (exercise_type IS NULL OR exercise_type <> 'photo') GROUP BY week_key ORDER BY week_key`,
          sql`SELECT player_name, COUNT(*)::int AS total FROM meals WHERE week_key = ${weekKey} AND (exercise_type IS NULL OR exercise_type <> 'photo') GROUP BY player_name ORDER BY total DESC`,
          sql`SELECT created_at FROM meals WHERE player_name = ${name} AND (exercise_type IS NULL OR exercise_type <> 'photo') ORDER BY created_at DESC OFFSET 1 LIMIT 1`,
          // Get latest workout description for this player (skip photo entries).
          sql`SELECT ai_description FROM meals WHERE player_name = ${name} AND (exercise_type IS NULL OR exercise_type <> 'photo') ORDER BY created_at DESC LIMIT 1`,
        ]);
      ...
      const history = historyResult.map((r) => ({
        weekKey: r.week_key as string,
        total: r.total as number,
      }));
      const leaderboard = leaderboardResult.map((r) => ({
        playerName: r.player_name as string,
        total: r.total as number,
      }));
      const latestDescription =
        workoutDescription || (descResult[0]?.ai_description as string | null) || null;
...
function checkWeeklyPersonalBest(
  weekCount: number,
  history: { weekKey: string; total: number }[],
  ...
  if (weekCount > previousBest && weekCount >= 3) {
    return {
      type: "weekly_pb",
      context: `${weekCount} workouts this week — a new personal best (previous record: ${previousBest})`,
    };
  }
  return null;
}

function checkLeaderboardTop3(
  normalizedName: string,
  leaderboard: { playerName: string; total: number }[],
): MilestoneMatch | null {
  ...
  if (position <= 2 && leaderboard.length >= 4) {
    ...
    return {
      type: "leaderboard",
      context: `moved into ${labels[position]} place on this week's leaderboard with ${leaderboard[position].total} workouts`,
    };
  }
  return null;
}
...
function checkGoalPace(weekCount: number): MilestoneMatch | null {
  ...
  if (isEarlyWeek && weekCount >= 4) {
    ...
    return {
      type: "pace",
      context: `${weekCount} workouts and it's only ${dayName} — way ahead of schedule`,
    };
  }
  return null;
}
...
function checkComeback(gapResult: any[], exerciseType: string): MilestoneMatch | null {
  ...
  if (daysSince >= 7) {
    return {
      type: "comeback",
      context: `back after ${daysSince} days away with a ${exerciseType} session`,
    };
  }
  return null;
}
