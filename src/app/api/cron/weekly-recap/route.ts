...
export async function GET(request: Request) {
  ...
  try {
    ...
    const realGoals = goals.filter(
      (g) => !testNames.has(g.playerName) && (g.cardioTarget > 0 || g.strengthTarget > 0),
    );

    const sql = getDb();

    // Get this week's stats per player. Photo-bingo uploads share the meals
    // table but are not workouts — they must be excluded from totals.
    ...
    // Get recent workout descriptions per player (skip photo-bingo entries).
    ...
    for (let i = 0; i < ranked.length; i++) {
      ...
      try {
        ...
        const goalMet =
          stats.cardio >= player.cardioTarget && stats.strength >= player.strengthTarget;
